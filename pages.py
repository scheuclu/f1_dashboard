...
def get_overview(races2data):
    cumpoints = pd.DataFrame(columns=conf.drivers, index=conf.races, data=0)
    for driver in conf.drivers:
        cumsum = 0
        for race in conf.races:
            p = races2data[race][driver]['Lukas points']
            if p != '':
                cumsum = cumsum + int(p)
...
def get_home(races2data):
    lukas_points = sum([df.loc['Lukas points'].replace('', '0').astype('int').sum() for df in races2data.values()])
    patrick_points = sum([df.loc['Patrick points'].replace('', '0').astype('int').sum() for df in races2data.values()])
    lisa_points = sum([df.loc['Lisa points'].replace('', '0').astype('int').sum() for df in races2data.values()])
    ...
    return dbc.Container([
        ...
        dbc.Progress(f'Patrick: {patrick_points}', value=patrick_points, color=conf.players['patrick']['color'],
                     **common_args),
...
def get_scoring():
    return [
        ...
        html.P(
            "For example, correctly guessing the driver on championship position 10 to win the race gives more points than guessing that the leader will win."),
        html.P(
            "Points are given if the race result of a driver is guesses within 3 places. Being spot on or just one-off gives extra points."),
...
def get_data(races2data):
    ...
    table = dash_table.DataTable(
        ...
        style_data_conditional=[{
            ...
            {
                'if': {
                    'filter_query': '{index} contains "points"',
                },
                'color': 'white',
                'fontWeight': 'bold'
            }
    ...
    dialog_success = dcc.ConfirmDialog(
        id='dialog_success',
        message='The access code was accepted. Youre updates are being saved.',
        displayed=False
    )
    dialog_failure = dcc.ConfirmDialog(
        id='dialog_failure',
        message='The access code was not recognized. Updates are not being saved.',
        displayed=False
    )
